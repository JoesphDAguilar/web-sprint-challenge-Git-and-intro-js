...
function get20s(array) {
  ...
  for(let i = 0; i < array.length; i++){
     if(array[i].years === "1907 - 1954" || array[i].years === "1904 - 1989"){
...
function artistByCountry(array, nat){
  const artByNat = [];
  for(let i = 0; i < array.length; i++){
    if(array[i].nationality === nat){
      artByNat.push(array[i].name);
    }
  }
  return artByNat;
}
